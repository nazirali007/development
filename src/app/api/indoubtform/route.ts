...
import { inDoubtSchema, bookingQueryPatchSchema } from "@/lib/zod-schema";
...
export async function POST(req: Request) {
	...
	try {
		const { name, phone, email, destination } = inDoubtSchema.parse(body);

		await prismadb.inquiry.create({
			data: {
				destination,
				email,
				name,
