...
import { getTripData, getRelatedTripData, categoryData } from "@/lib/express";

export const dynamic = "force-dynamic"

export async function generateMetadata() {
	const strapiContent = await getStrapiTripPageContent({ slug: "almaty-tour-package-from-delhi" });
...
export default async function Page() {
	...
	const tripData = await getTripData({ slug: "almaty-tour-package-from-delhi" });
	...
	const relatedTripData = await categoryData({ slug: "almaty-tour-package" })
	const { trips } = relatedTripData
	// let relatedTripData = relatedtrips.data.map((trip: any) => {
	// 	return {
	// 		id: trip.id,
	...
	// console.log('related trips', relatedTripData);
	...
	return (
		<>
			...
			<div className='flex flex-col justify-center'>
				{/* About */}
				<div className='mb-20 flex w-full lg:w-[90vw] flex-col items-center justify-center gap-8 px-8  md:flex-row md:justify-between'>
